...
public class FlinkStreamingJob {
	...
	String inputStreamName = "input";
	String outputStreamName = "output";
        ...
        RandomCutForestOperator<InputData, OutputData> randomCutForestOperator =
                RandomCutForestOperator.<InputData, OutputData>builder()
                        ...
                        .setShingleSize(1)
                        .setSampleSize(10)
                        .setInputDataMapper((inputData) -> new float[]{inputData.getSystolic(), inputData.getDiastolic()})
                        .setResultMapper(((inputData, score) -> new OutputData(inputData.getNo(),inputData.getSystolic(), inputData.getDiastolic(), score)))
                        .build();

        DataStream<OutputData> outputDataStream = source
                .process(randomCutForestOperator, TypeInformation.of(OutputData.class)).setParallelism(1);
	outputDataStream.print();
        outputDataStream.sinkTo(createSink(outputStreamName, region));
